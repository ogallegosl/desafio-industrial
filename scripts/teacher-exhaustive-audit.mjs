...
check('Estructura', '28 migraciones presentes', migrations.length === 28, 'high', `detectadas=${migrations.length}`)
check('Estructura', 'Bootstrap incluye 28 migraciones', migrations.every(m=>bootstrap.includes(m)), 'high')
check('Estructura', 'Migración RLS E2E es la última', migrations.at(-1) === '0028_importer_rls_runtime_hotfix.sql', 'high', migrations.at(-1))
